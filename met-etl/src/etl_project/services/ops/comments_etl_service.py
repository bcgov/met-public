...
@op(out={"comments_last_run_cycle_datetime": Out(), "comments_new_run_cycle_id": Out()})
def get_comments_last_run_cycle_time(context, flag_to_trigger_comments_etl):
    ...
    for last_run_cycle_time in comments_last_run_cycle_datetime:
        ...
        for run_cycle_id in max_run_cycle_id:
            ...
            met_etl_db_session.add(EtlRunCycleModel(id=new_run_cycle_id, packagename='userfeedback', startdatetime=datetime.utcnow(), 
                        enddatetime=None, description='started the load for table user_feedback', success=False))
...
@op(out={"new_comments": Out(), "comments_new_run_cycle_id": Out()})
...
@op(out={"comments_new_run_cycle_id": Out()})
...
@op(out={"flag_to_run_step_after_comments": Out()})
def comments_end_run_cycle(context, comments_new_run_cycle_id):
    ...
    met_etl_db_session.query(EtlRunCycleModel).filter(
        ...
        EtlRunCycleModel.success == False).update({'success': True, 'description': 'ended the load for table user_feedback'})
