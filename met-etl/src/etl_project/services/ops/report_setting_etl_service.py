...
@op(required_resource_keys={"engagement_db_session",
    "etl_db_session"},
    out={"setting_last_run_cycle_datetime": Out(),
         "setting_new_run_cycle_id": Out()})
def get_setting_last_run_cycle_time(context, flag_to_run_step_after_survey):
    ...
    setting_last_run_cycle_datetime = etl_db_session.query(
        func.coalesce(
            func.max(
                EtlRunCycleModel.enddatetime),
            default_datetime)).filter(
...
# extract the report settings that have been created or updated after the
# last run
@op(required_resource_keys={"engagement_db_session",
    "etl_db_session"},
    out={"new_setting": Out(),
    "updated_setting": Out(),
         "setting_new_runcycleid": Out()})
def extract_setting(
        context,
        setting_last_run_cycle_time,
        setting_new_runcycleid):
...
# load the report setting created or updated after last run to the
# analytics database
@op(required_resource_keys={"engagement_db_session",
    "etl_db_session"},
    out={"setting_new_runcycleid": Out()})
def load_setting(
        context,
        new_setting,
        updated_setting,
    ...
    if len(all_settings) > 0:
        ...
        for setting in all_settings:
            # get the survey id from analytics database based on the source
            # system survey id
            analytics_survey_data = etl_db_session.query(EtlSurveyModel)\
            ...
            etl_db_session.query(EtlRequestTypeOptionModel).filter(
...
@op(required_resource_keys={"engagement_db_session",
    "etl_db_session"},
    out={"flag_to_run_step_after_setting": Out()})
