...
@op(required_resource_keys={"engagement_db_session",
    "etl_db_session"},
    out={"user_details_last_run_cycle_datetime": Out(),
         "user_details_new_run_cycle_id": Out()})
def get_user_last_run_cycle_time(context):
    ...
    user_details_last_run_cycle_datetime = etl_db_session.query(
        func.coalesce(func.max(EtlRunCycleModel.enddatetime), default_datetime)).filter(
        EtlRunCycleModel.packagename == 'userdetails', EtlRunCycleModel.success).first()
...
@op(required_resource_keys={"engagement_db_session",
    "etl_db_session"},
    out={"new_participants": Out(),
    "updated_participants": Out(),
         "user_details_new_run_cycle_id": Out()})
def extract_participant(
        context,
        user_details_last_run_cycle_datetime,
        user_details_new_run_cycle_id):
...
@op(required_resource_keys={"engagement_db_session",
    "etl_db_session"},
    out={"user_details_new_run_cycle_id": Out()})
def load_user(
        context,
        new_participants,
        updated_participants,
    ...
    if len(all_participants) > 0:
        ...
        for participant in all_participants:
            session.query(EtlUserDetailsModel).filter(
                EtlUserDetailsModel.name == participant.email_address).update({'is_active': False})
            user_model = EtlUserDetailsModel(
                name=participant.email_address,
                is_active=True,
                created_date=participant.created_date,
                updated_date=participant.updated_date,
                runcycle_id=user_details_new_run_cycle_id)
...
@op(required_resource_keys={"engagement_db_session",
    "etl_db_session"},
    out={"flag_to_run_step_after_user": Out()})
