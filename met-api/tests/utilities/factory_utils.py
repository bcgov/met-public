...
def factory_metadata_requirements(auth: Optional[Auth]=None):
    ...
    factory_staff_user_model(TestJwtClaims.staff_admin_role['sub'], TestUserInfo.user_staff_1, tenant.id)
...
def factory_taxon_requirements(auth: Optional[Auth]=None):
    ...
    factory_staff_user_model(TestJwtClaims.staff_admin_role.get('sub'), 
                             TestUserInfo.user_staff_1, tenant.id)
    if auth:
...
def factory_staff_user_model(external_id=None, user_info: dict = TestUserInfo.user_staff_1, tenant_id=None):
